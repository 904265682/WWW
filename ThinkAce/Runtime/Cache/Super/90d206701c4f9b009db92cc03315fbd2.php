...
<div id="sidebar" class="sidebar responsive">
	...
	<li class="hidden">
		<a href="<?php echo (SITE_URL); ?>Super/AuthAndRole/rolePower">
			权限编辑
		</a>
	</li>
</ul>
					...
					<thead>
						<tr>
								<td style="text-align:left;" colspan="2">
									角色名称：<?php echo ($role_info["role_name"]); ?>
								</td>
						</tr>
					</thead>
					<!-- 重新赋值 -->
				<!-- 	<?php $per_ids=$role_info["role_auth_ids"]; $per_ids=explode(",",$per_ids); unset($per_ids[0]); $num=count($per_ids); $str=''; for($i=0;$i<$num;$i++){ $str.=$per_ids[$i+1].','; } $str=substr($str,0,-1); $rst=explode(",",$str); ?> -->
					<form action="<?php echo (SITE_URL); ?>Super/AuthAndRole/rolePower/role_id/<?php echo ($role_info["role_id"]); ?>" class="form-horizontal" role="form" method="post">
					<?php if(is_array($auth_infoA)): foreach($auth_infoA as $key=>$v): ?><tr>
								<td width="15%" class="left">
									<!-- <?php if(in_array($v.auth_id,$have_authids)): ?><input name="auth_id[]" type="checkbox" checked class="ace par" value="<?php echo ($v["auth_id"]); ?>" checked="true" />
									<?php else: ?> -->
										<input name="auth_id[]" type="checkbox" <?php if(in_array($v['auth_id'],$have_authids)): ?>checked<?php endif; ?> class="ace par" value="<?php echo ($v["auth_id"]); ?>"/>
									<!--<?php endif; ?> -->
										<span class="lbl">&nbsp;<kbd><?php echo ($v["auth_name"]); ?></kbd></span>
								</td>
								<td class="ace1">
									<?php if(is_array($auth_infoB)): foreach($auth_infoB as $key=>$v1): ?><!-- <?php if(in_array($v1['auth_id'],$have_authids)): ?><input name="auth_id[]" type="checkbox" checked="true" class="ace cld" value="<?php echo ($v1["auth_id"]); ?>|<?php echo ($v1["name"]); ?>|<?php echo ($v1["con_name"]); ?>|<?php echo ($v1["ca_path"]); ?>|"/>
											<?php else: ?>
												<input name="auth_id[]" type="checkbox" class="ace cld"  value="<?php echo ($v1["auth_id"]); ?>|<?php echo ($v1["name"]); ?>|<?php echo ($v1["con_name"]); ?>|<?php echo ($v1["ca_path"]); ?>|"/><?php endif; ?> -->
											<?php if(($v["auth_id"]) == $v1["auth_pid"]): ?><input name="auth_id[]" type="checkbox" <?php if(in_array($v1['auth_id'],$have_authids)): ?>checked<?php endif; ?> class="ace cld" value="<?php echo ($v1["auth_id"]); ?>"/>
											&nbsp;
												<span class="lbl"><?php echo ($v1["auth_name"]); ?></span>&nbsp;&nbsp;<?php endif; endforeach; endif; ?>
								</td>
...
<script type="text/javascript">
	jQuery(document).ready(function() {
		...
		$('.delete-checked').click(function(e) {
			...
			var href = "<?php echo (SITE_URL); ?>index.php/Super/SuperUser/Index/id" + '/' + ids;
			location.href = href;
		});
